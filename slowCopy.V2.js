...
bug: 同目录下移动文件时，会导致文件被删除。

1. 速度控制由整体计算改为过去 10 秒的平均速度，减少卡顿导致后续补速的情况

2. 块大小根据设置的速度自动调整，使更新频次保持在 10 次/秒左右

3. 优化输出信息，显示实时速度，显示剩余时间

4. 根据 screen 宽度调整进度条长度

5. 进度条改为更细致层级，类似 conda 的进度条

6. 内部建立缓冲区，减少频繁的写操作。并分离屏幕输出和文件读写的绑定。
...
function parseSource(multisrcs) {
